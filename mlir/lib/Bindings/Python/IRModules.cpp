...
public:
  ...
  void insert(int index, PyOperation &newOperation) {
    ...
    // mlirBlockInsertOwnedOperation. Add a C-API to add an operation relative
    // to another and then handle the indexing here.
    // Alternatively, consider making mlirBlockInsertOwnedOperation return a
    // failing status for out of bounds.
    if (index > dunderLen()) {
      throw SetPyError(PyExc_IndexError,
...
void mlir::python::populateIRSubmodule(py::module &m) {
      ...
      .def(
          ...
          [](PyMlirContext &self, std::string name, PyLocation location,
            ...
            MlirOperationState state =
                mlirOperationStateGet(name.c_str(), location.loc);
            ...
            if (!mlirAttributes.empty()) {
              ...
              mlirOperationStateAddAttributes(&state,
                                              mlirNamedAttributes.size(),
            ...
            return PyOperation::createDetached(self.getRef(), operation)
                .releaseObject();
          },
          py::arg("name"), py::arg("location"), py::arg("results") = py::none(),
  ...
  py::class_<PyRegion>(m, "Region")
      .def_property_readonly(
          "blocks",
          [](PyRegion &self) {
            return PyBlockList(self.getOperation(), self.get());
          },
      ...
      .def(
          ...
          [](PyRegion &self) {
            ...
            return PyBlockIterator(self.getOperation(), firstBlock);
          },
  ...
  py::class_<PyBlock>(m, "Block")
      .def_property_readonly(
          "operations",
          [](PyBlock &self) {
            return PyOperationList(self.getOperation(), self.get());
          },
      ...
      .def(
          ...
          [](PyBlock &self) {
            ...
            return PyOperationIterator(self.getOperation(), firstOperation);
          },
