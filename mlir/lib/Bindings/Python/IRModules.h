...
/// Operations exist in either an attached or detached state. In the detached
/// state (as on creation), an operation is owned by the creator and its
/// lifetime extends either until its reference count dropping to zero or it
/// being attached to a parent.
/// When attached to a parent, the operation will capture a reference to its
/// parent PyOperation. Attached operations are valid until they are removed
/// by their parent or a bulk IR modification takes place.
class PyOperation;
